...
function hexToBase64(hexStr) {
    try {
        ...
        // 将字节数组转换为Base64字符串
        const base64Str = btoa(String.fromCharCode.apply(null, bytes))

        return base64Str
    } catch (e) {
        console.error('16进制转Base64出错', e)
    }
}
...
function convertKeysToLowercase(obj) {
    ...
    Object.keys(obj).forEach(key => {
        ...
        convertedObj[newKey] = convertKeysToLowercase(obj[key])
    })
    return convertedObj
}
// obj键名 蛇形命名法转大驼峰
function lowercaseToConvertKeys(obj) {
    if (typeof obj !== 'object' || obj === null) {
        return obj
    }
    if (Array.isArray(obj)) {
        return obj.map(lowercaseToConvertKeys)
    }
    ...
    Object.keys(obj).forEach(key => {
        ...
        const newKey = words
            ...
            .join('')
        convertedObj[newKey] = lowercaseToConvertKeys(obj[key])
    })
    return convertedObj
}
...
// 加解密类
function DESUtil(option) {
    const { secretKey, mode } = option
    const { key, iv } = secretKey
    // 加密
    const encrypt = (data, { transJson, isTransHex }) => {
        if (transJson) {
            try {
                data = JSON.stringify(data)
            } catch (e) {
                console.error('转JSON格式失败', e)
            }
        }
        return handleEncrypt(data, isTransHex)
    }
    // // 解密
    const decrypt = (data, { isTransBase64, transObj }) => {
        const decryptData = handleDecrypt(data, isTransBase64)
        if (transObj) {
            try {
                return JSON.parse(decryptData)
            } catch (e) {
                console.error('转Object格式失败', e)
            }
        }
        return decryptData
    }

    function handleEncrypt(text, isTransHex) {
        ...
        // 加密后的数据
        const encryptData = sm4Util[mode].encryptSM4(text, key, iv)
        if (isTransHex) {
            return base64ToHex(encryptData)
        }
        return encryptData
    }
    // 解密方法
    const handleDecrypt = (text, isTransBase64) => {
        if (isTransBase64) {
            text = hexToBase64(text)
        }
        // 返回解密后的数据
        return sm4Util[mode].decryptSM4(text, key, iv)
    }
...
const { encrypt, decrypt } = DESUtil(encryptConfig)
